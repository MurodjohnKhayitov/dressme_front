        ...
        {personItems
          ...
          .map((data) => {
            return (
              <div
                key={data?.id}
                className="w-fit ss:hidden md:block md:flex items-center  "
              >
                <button
                ...
                <button
                  className={`mr-1 ${genderStyle}  w-fit h-[44px] px-[16px] justify-between mr-2 flex items-center bg-btnBgColor border border-searchBgColor rounded-lg`}
                >
                  <span className="mr-3">
                    <MdAdd />
                  </span>
                  <span className=" mt-1 not-italic font-AeonikProMedium text-sm leading-4 tracking-[0,16px]">
                    Добавить персонажа
                  </span>
                </button>
                <button
